...
void AnnotatedCameraWidget::drawLead(QPainter &painter, const cereal::RadarState::LeadData::Reader &lead_data, const QPointF &vd, const bool adjacent) {
  painter.save();
  ...
  if (s->scene.world_objects_visible && !showDriverCamera) {
    ...
    if (s->scene.longitudinal_control && sm.rcv_frame("radarState") > s->scene.started_frame && !scene.hide_lead_marker) {
      ...
      if (lead_left.getStatus()) {
        drawLead(painter, lead_left, s->scene.lead_vertices[2], true);
      }
      if (lead_right.getStatus()) {
        drawLead(painter, lead_right, s->scene.lead_vertices[3], true);
      }
      if (lead_left_far.getStatus()) {
        drawLead(painter, lead_left_far, s->scene.lead_vertices[4], true);
      }
      if (lead_right_far.getStatus()) {
        drawLead(painter, lead_right_far, s->scene.lead_vertices[5], true);
      }
    }
  }
